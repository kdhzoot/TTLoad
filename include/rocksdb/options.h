...
struct IngestExternalFileOptions {
  ...
  // S3 ingestion mode: Set to true to enable S3-specific ingestion behavior.
  // When enabled, files will be ingested to the level specified by
  // s3_target_level.
  bool s3_ingest = false;

  // Target level for S3 ingestion. Only used when s3_ingest is true.
  // Files will be ingested directly to this level if it fits.
  int s3_target_level = -1;
};
